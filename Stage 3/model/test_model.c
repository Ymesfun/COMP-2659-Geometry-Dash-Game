...
void test_spike_initialization(Model model, int *failcount){
    ...
    for(int i = 0;i<model.Spikecount; i++){
...
void test_block_initialization(Model model,int *failcount){
    ...
    for(int i = 0;i<model.Blockcount;i++){
        if(model.Blocks[i].x == i*100 && model.Spikes[i].y == i*100 && model.Spikes[i].dx == i*2 && model.Spikes[i].dy == 0){
...
void test_model_initialization(){
    ...
    test_player_status(model,&failcount);
...
void test_player_jump(Model model, int *failcount){
    ...
    if(model.Player.entity.x == 180 && model.Player.entity.y == 150 && model.Player.entity.dx == 10 && model.Player.entity.dy == 0){
...
void test_spike_move(Model model, int *failcount){
    ...
    for(int i = 0;i<5;i++){
        for(int i = 0;i<model.Spikecount;i++){
            object_move(&model.Spikes[i]); /*More efficient algo that isnt O(n^2) but this is two small iterations, so why not?*/
        }
    }
...
void test_goal_move(Model model, int *failcount){
    ...
    if(model.Goal.x == 50 && model.Goal.y == 160 && model.Goal.dx == 10 && model.Goal.dy == 0){
...
void test_movement(){
    ...
    int failcount;
    model.Spikecount = num_spikes;
    model.Blockcount = num_spikes;
    initialize_player_gameobj(&model.Player,50,150,10); /*This uses DRY - dont wanna repeat myself for jump/move intitializations*/
    ...
    test_spike_movement(model, &failcount);
    printf("Block move test\n");
    test_block_movement(model,&failcount);
    printf("Goal move test\n");
    test_goal_movement(model,&failcount);
    if(failcount == 0){
        printf("All movement tests passed!\n");
    }else if(failcount<7){
        printf("%d movement tests failed!",failcount);
    }else{
...
void test_spike_collision(Model model, int*failcount){
    ...
    for(int i=0;i<model.Spikecount;i++){    /*Test if direct collisions are working*/
        if(object_collisioncheck (&model.Player, &model.Spikes[i])){
            printf("Spike collision %d working correctly",i);
        }else{
            printf("Spike collision %d is not working!",i);
            (*failcount)++;
        }
    }
    model.Spikes[0].x +=2;/*Miss where object is to right*/
    model.Spikes[1].y +=2;/*Miss where object is below*/
    for(int i = 0; i<model.Spikecount;i++){
        if(!object_collisioncheck(&model.Player,&model.Spikes[i])){
            printf("Object %d down/right miss collision working successfully.",i);
        }else{
            printf("Object %d down/right miss collision not working!",i);
            (*failcount)++;
        }
    }
    model.Spikes[0].x = 48; /*Miss where object is to left*/
    model.Spikes[1].y = 17; /*Miss where object is above */
    for(int i = 0; i<model.Spikecount;i++){
        if(!object_collisioncheck(&model.Player,&model.Spikes[i])){
            printf("Object %d left/up miss collision working correctly.",i);
        }else{
            printf("Object %d up/left miss collision not working!",i);
            (*failcount)++;
        }
    }
}
...
void test_wall_collision(Model model, int *failcount){
    for(int i = 0; i<model.Blockcount;i++){
         initialize_wall_gameobj(&model.Blocks[i],82+i,50,0);
    }
...
void test_collisions(){
    ...
    model.Spikecount = num_spikes;
    int failcount = 0;
    initialize_player_gameobj(&model.Player,50,50,0);
    ...
    if(failcount == 0){
        ...
        printf("%d collision tests failing!",failcount);
    }else{
        printf("All tests failing! This is %d tests!",failcount);
    }
}
/*******************************************************************************
